...
class UserService extends Service {
    ...
    public function confirm($token_code) {
        try {
            $token      = new Token();
            $token_info = $token->where('token', $token_code)
                                ->where('status', 0)
                                ->first();
            if (!$token_info) {
    ...
    public function change_email($params = []) {
        try {
            ...
            $email     = $params['email'];
            $user      = new User();
            $old_email = $user->find_id($id)['email'] !== null ? $user->find_id($id)['email'] : '';
    ...
    public function insert_and_send_mail_activate_acc($data, $result) {
        if ($result['error'] == false) {
            ...
            $username_check = $user->where('username', $data['username'])
                                   ->first();
            if ($username_check) {
                $result['error']     = true;
            ...
            $email_check = $user->where('email', $data['email'])
                                ->first();
    ...
    public function send_mail_change_email($id, $email, $old_email) {
        ...
        $message = "Click <a href='" . Env::APP_URL . "user/confirm/$token_code'>here</a>
 				to agree to change email for $email \n " . " If you want to change to another email, please send it after: " . Env::EXPIRE_TIME . " seconds";
        $headers = get_mail_header();
